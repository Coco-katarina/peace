...
import { getHdfsDirStats, getProjectList, getStrucList, getCedian, getNetdiskFiles, deleteFile, collectedFile, removeFile, downloadFile } from '../actions/dataFiles';
                    ...
                    if (downloadFileRes || deleteFileRes) {
                        ...
                        dataSource.push({
                            ...
                            operation: <span>
                                {downloadFileRes ?
                                    <a onClick={()=>{
                                        window.open(downloadFile(encodeURIComponent(fileData[k].link),encodeURIComponent(fileData[k].name)))
                                    }} >下载</a> : null}
...
function mapStateToProps(state) {
    const { netdiskFiles, auth, projectList, strucList, dataFile, dataFileStats ,global, cedian } = state;
    let isRequesting = netdiskFiles.isFetching || projectList.isRequesting;
    return {
        user: auth.user,
